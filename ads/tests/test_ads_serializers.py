...
class CustomerProfileSerializerTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="profuser", password="pwd", mobile_number = "+239995555")
        self.user.save()
...
class CategorySerializerTest(TestCase):
    def test_category_serialization_output(self):
        ...
        category = Category.objects.create(name="Carros", slug="carros", icon="🚗", description="Auto")
...
class AdCreateSerializerValidationTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="profuser2", password="pwd2", mobile_number = "+2399955552")
        self.user.save()
    ...
    def test_validation_fails_missing_product_name(self):
        ...
        with self.assertRaises(ValidationError) as context:
            serializer.is_valid(raise_exception=True)
        self.assertIn("Temp AD or product name not found!", context.exception.detail['detail'])
...
class AdCreateSerializerCreationTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="gamer", password="pwd", mobile_number = "+2399955552")
        self.user.save()
        ...
        self.mock_file = SimpleUploadedFile("ps5.jpg", b"data", content_type="image/jpeg")
        TemporaryAdImage.objects.create(temporary_ad=self.temp_ad, image=self.mock_file)
