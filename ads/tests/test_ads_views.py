...
def create_user(django_user_model):
    ...
    def make_user(username="testuser", password="password123",mobile_number = "+239995555" ):
        return django_user_model.objects.create_user(username=username, password=password, mobile_number=mobile_number)
    return make_user

@pytest.fixture
def customer_profile(create_user):
    """Creates a regular testing user accompanied by a profile."""
    user = create_user(username="customer_user", password="password123", mobile_number = "+239995555" )
    return user.profile

@pytest.fixture
def other_customer_profile(create_user):
    """Creates an isolated second profile to test query separation."""
    user = create_user(username="other_user",password="password223", mobile_number = "+239994555")
    return user.profile
...
class TestAdManageViewSet:
    ...
    def test_queryset_isolation_prevents_viewing_others_ads(self, api_client, customer_profile, other_ad):
    ...
    def test_user_cannot_modify_or_delete_others_ads(self, api_client, customer_profile, other_ad):
        api_client.force_authenticate(user=customer_profile.user)
...
class TestAdImageViewSet:
    ...
    def test_list_nested_images_publicly(self, api_client, official_ad):
        ...
        image = AdImage.objects.create(ad=official_ad, image="ads/test.jpg")
...
class TestTemporaryAdImageViewSet:
    ...
    def test_anonymous_user_can_upload_temporary_ad_image(self, api_client, temporary_ad):
        # 1. Reverse the URL using the temporary_ad's true UUID primary key string
        url = reverse('ads:temporary-ad-image-list', kwargs={'temporary_ad_pk': str(temporary_ad.pk)})
        ...
        mock_image = SimpleUploadedFile(
            name="guest_photo.gif", 
            content=small_gif_bytes, 
    ...
    def test_list_nested_temporary_images(self, api_client, temporary_ad):
        TemporaryAdImage.objects.create(temporary_ad=temporary_ad, image="guest/img.png")
        
        url = reverse('ads:temporary-ad-image-list', kwargs={'temporary_ad_pk': temporary_ad.pk})
        response = api_client.get(url)
